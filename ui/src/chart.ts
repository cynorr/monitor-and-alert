...
export class Panel {
    ...
    render(data: ChartData) {
        ...
        if (rebuild) {
            ...
            for (const [name, value] of Object.entries(data.indicator_preview))
                this.lines[name].update(value as LineData);
        }
    ...
    showOHLC(row?: Row) { $(this.id + '-ohlc').textContent = row ? `O ${money(row.open)}  H ${money(row.high)}  L ${money(row.low)}  C ${money(row.close)}  V ${compact(row.volume)}` : '—'; }
    reveal(row: Row) {
