...
正常状态在 UI 中静默；后台仍检查。已进入正常运行的周期，bar_end 超过 15 秒尚未获得有效 bar 时显示 warning，恢复后清除。初始旧历史补齐只显示加载。
...
独立 simulator 提供 Quote 和一致的五周期 closed bars，通过实例级 clock 与行情对象复用 data 调度/验证，只写临时数据库。生产服务不导入模拟器、不使用模拟分支绕过校验。

默认测试离线，不连接券商。浏览器模拟入口由 simulator/ 维护，tests/preview_fixture.py 只是兼容启动入口；数据与数据库为临时合成样本，不是正式行情。真实接口测试需独立明确范围与时限；历史实测报告不代表本轮已 live 验证。
