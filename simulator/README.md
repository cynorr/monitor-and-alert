...
模拟器按 workspace focus/wait 生成 Quote 和五周期 closed bars，复用实际调度、校验、指标、2h/4h 合成与同一 UI。使用实例级交易时钟，不修改系统时钟。行情数据只在临时 SQLite，列表操作只写入启动时复制的临时 workspace.json，退出删除；新增返回模拟证券信息，不表示 Longbridge 验证通过；不读取凭证、不连 Longbridge。原额外 18766 Quote 监听已删除。
